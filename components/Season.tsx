...
export default function Season() {
    ...
    try {
      ...
      if (twa?.openInvoice) {
        twa.openInvoice(data.invoiceLink, (status: string) => {
          if (status === 'paid') {
            showToast('Payment successful! Item applied to your account.')
  ...
  return (
    <div className="animate-page px-3 pt-3">
      <div className="rounded-md p-4 mb-3 text-center" style={{ background: 'linear-gradient(135deg, #0d0820, #120d28)', border: '1px solid #2a1a50' }}>
        <div className="font-mono mb-1" style={{ fontSize: 9, color: 'var(--rtm-muted)', letterSpacing: '3px' }}>SEASON 01 · $RTM GENESIS EPOCH</div>
        <div className="font-head font-bold glow-purple" style={{ fontSize: 26, color: 'var(--rtm-purple)' }}>ROTUM PROTOCOL</div>
        <div className="font-mono mt-1" style={{ fontSize: 10, color: 'var(--rtm-muted)' }}>Compete globally. Earn $RTM. Dominate the network.</div>
        <div className="font-mono font-bold mt-3" style={{ fontSize: 24, color: 'var(--rtm-green)' }}>
          {pool.toLocaleString()}<span style={{ fontSize: 13, color: 'var(--rtm-accent)', marginLeft: 4 }}>$RTM</span>
        </div>
        <div className="progress-track mt-2 mx-4"><div className="progress-fill-green" style={{ width: `${barPct}%` }} /></div>
        <div className="font-mono mt-2" style={{ fontSize: 12, color: 'var(--rtm-amber)' }}>ENDS IN: {countdown || '—'}</div>
      </div>
      ...
      <div className="rtm-card mb-3 overflow-hidden" style={{ borderTop: '2px solid var(--rtm-amber)' }}>
        <div className="grid font-mono px-3 py-1.5" style={{ gridTemplateColumns: '2fr 1fr 1fr 1.5fr', fontSize: 9, color: 'var(--rtm-muted)', letterSpacing: '1px', borderBottom: '1px solid var(--rtm-border)' }}>
          <span>TIER</span><span>OPS</span><span>SHARE</span><span className="text-right">AVG REWARD</span>
        </div>
        {REWARD_TIERS.map(row => (
          <div key={row.tier} className="grid font-mono px-3 py-2" style={{ gridTemplateColumns: '2fr 1fr 1fr 1.5fr', fontSize: 11, color: 'var(--rtm-text)', borderBottom: '1px solid var(--rtm-border)' }}>
            <span>{row.tier}</span>
          ...
          return (
            <div key={item.slug} className="rtm-card flex items-center gap-3 px-3 py-2.5">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-head font-semibold" style={{ fontSize: 13, color: 'var(--rtm-text)' }}>{item.name}</div>
                <div className="font-mono mt-0.5" style={{ fontSize: 9, color: 'var(--rtm-muted)' }}>{item.desc}</div>
              </div>
              <button
